...
st.title("Калькулятор рассрочек с равными платежами (аннуитет) и уменьшением долга")
...
for month in range(1, months + 1):
    interest = remaining * monthly_rate            # процент с остатка
    principal = monthly_payment - interest        # часть платежа на погашение основного долга
    remaining -= principal                         # уменьшаем остаток долга
    schedule.append({
        "Месяц": month,
        "Платеж": monthly_payment,
        "Основной долг": principal,
        "Проценты": interest,
        "Погашение долга": principal,            # сколько реально уменьшилось
...
total_principal = sum(item["Основной долг"] for item in schedule)
...
st.write(f"Общая сумма основного долга: {total_principal:,.2f} руб.")
